...
function cargarDatos() {
    ...
    if ($.fn.DataTable.isDataTable('#tbProductoTienda1')) {
        $('#tbProductoTienda1').DataTable().destroy();
    }

    $('#tbProductoTienda1 tbody').html('');
        ...
        function (response) {
            if (response.estado) {
                ...
                $.each(response.objeto, function (i, row) {
                    ...
                    ).appendTo("#tbProductoTienda1 tbody");

                })
            }

            table = $('#tbProductoTienda1').DataTable({
                responsive: true
            });
        },
        function () {
...
$('#tbProductoTienda1 tbody').on('click', 'button[class="btn btn-sm btn-danger"]', function () {
...
$('#btnAsignar').on('click', function () {
    ...
    if ($("#txtIdTienda").val() == "0" || $("#txtIdProductos").val() == "0") {
        camposvacios = true;
    }
    ...
    if (!camposvacios) {
        ...
        var request = {
            oProductoTienda: {
                oProducto: { IdProducto: parseInt($("#txtIdProductos").val()) },
...
$('#tbProductos tbody').on('click', 'button[class="btn btn-sm btn-primary ml-2"]', function () {

    var model = $(this).data("producto")
    $("#txtIdProductos").val(model.IdProducto);
    $("#txtCodigos").val(model.Codigo);
    $("#txtNombres").val(model.Nombre);
    $("#txtDescripcions").val(model.Descripcion);
